...
public final class AbstractNodeMacroNamespace__BehaviorDescriptor extends BaseBHDescriptor {
  ...
  /*package*/ static SNode getTargetMacroOfConceptFrom_id2vVmcK3rHVe(@NotNull SAbstractConcept __thisConcept__, final SAbstractConcept macroConcept, SNode parent) {
    ...
    while (ancestor != null && !(ListSequence.fromList(new IAttributeDescriptor.NodeAttribute(CONCEPTS.NodeMacro$qU).list(ancestor)).any(new IWhereFilter<SNode>() {
    ...
    return ListSequence.fromList(new IAttributeDescriptor.NodeAttribute(CONCEPTS.NodeMacro$qU).list(ancestor)).findFirst(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return SNodeOperations.isInstanceOf(it, SNodeOperations.asSConcept(macroConcept));
      }
    });
